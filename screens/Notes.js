...
const Notes = (props) => {
    ...
    const [varNote, setVarNote] = useState(false)
  	...
  	const fetchData = () => {
		...
		.then(res =>{
			...
			fetch(`https://agile-bastion-40085.herokuapp.com/${res.institute}/${res.dept}/${course}/notes`)
			.then(res=>res.json())
			.then(results=>{
				setData(results)
				setLoading(false)
	...
	const addnotes = () => {
		props.navigation.navigate("AddNotes",{course, role})
    }

	return(
        <View style={{flex:1, backgroundColor:"white"}}>
			<MyComponent backBtn={true} titleText="Notes" goback={goback}/>
			<Text style={styles.closeButton}>Choose among these Notes of : {course}</Text>
            <FlatList
                data = {data}
                renderItem = {({item})=>
                <TouchableOpacity onPress={ () => {makeDownload(item)}}>
					<View style={styles.ordersView}>
						...
						<View style={{justifyContent: 'center', flex: 1, alignItems:'center'}}>
							<Button icon="file-download" labelStyle={{ fontSize: 40}} ></Button>
